...
| 2026-04-23 | Stale-PID detection now cross-checks the kernel's `create_time()` against the epoch stored in the pid file, so a recycled PID whose cmdline happens to contain the expected substring is no longer mis-classified as live. Top-bar market pill now reads from a `market_status` settings row written every minute by a new `poll_market_status_job`. Added `/debug/env` diagnostic endpoint exposing the env file's size, encoding, BOM status, keys parsed, and a masked preview of the access token (bound to 127.0.0.1). `stop.bat` hardened with image-name + module-cmdline fallback kills when the title-based taskkill doesn't catch a detached console. | Windows runtime report: "worker already running as pid …" + "market unknown" + token still showed as missing after BOM fix deployed. |
| 2026-04-23 | Default web port moved from 8765 to 18765 to avoid the common JupyterLab / older-Dash collision. Override via `EMRB_WEB_PORT` in `.env` remains. `run.bat`, `run.sh`, `README.md`, `.env.example` updated. | User port collision on Windows host. |
