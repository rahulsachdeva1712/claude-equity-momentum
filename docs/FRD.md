...
Last updated: 2026-04-28 (Today's Change excludes carried-over unrealized).
...
| 2026-04-26 | Unrealized P&L tile vs. Current Paper Trading Book row still diverged after the previous "make _summary use book_rich" fix — now via a `live_ltp` race. The request handler called `book_rich` twice: once inside `summary_rich` for the KPI tile, and once directly for the `book` template var. If the worker's `ltp_poll_job` wrote `live_ltp` between those two reads, the KPI saw one snapshot (e.g. GUJCOTEX 11.24 → +89.43) and the table saw another (11.20 → −268.29). Fixed by computing `book_rich` exactly once per request in `app/web/main.py` and threading it through `summary_rich(..., book=…)` → `_summary(..., book=…)`. The KPI tile and the per-row sums now key off a single in-memory list, so they're guaranteed to agree on whatever instant the request was served. New regression: `tests/test_paper_engine.py::test_summary_uses_provided_book_avoiding_live_ltp_race` mutates `live_ltp` between the snapshot and the `summary_rich` call and asserts the tile reflects the pre-mutation snapshot. Suite 165/165. | User feedback: "unrealized pnl is still not matching in KPI and Current Paper Trading Book table". |
| 2026-04-28 | "Today's Change" KPI tile was treating *all running unrealized* as if it happened today. On a buy-and-hold day with zero intraday move, a position carried over from a prior session at avg 11.23 / current 11.21 read `Today's Change = -178.86` (the running unrealized vs. avg_cost) when the truthful answer is ~₹0 — yesterday's loss was being re-counted as today's. Fixed in `web.views._summary` by introducing `prior_eod_unrealized = paper_pnl_daily.unrealized` for the most recent session BEFORE today, and computing `today_change = today_realized + (today_unrealized - prior_eod_unrealized)`. The "Unrealized P&L" tile and the Current Book unrealized column still show **running** unrealized (so they retain table↔tile parity); only "Today's Change" subtracts the carry-over. Also tightened `today_realized` to read the row whose `session_date == session_date_for(now)` instead of the bare "latest" row, so a day with no fills (and no refresh-job row yet) reads 0 instead of yesterday's realized. New regressions: `tests/test_paper_engine.py::test_today_change_excludes_carried_over_unrealized` (zero intraday move ⇒ today_change == 0) and `test_today_change_captures_intraday_move` (price climbs 90→95 ⇒ today_change = +50, today_unrealized = -50 running). Suite 167/167. | User feedback: "why is it still showing yesterday order's as today's? I thought you did the health check?" |
| 2026-04-26 | Live tab parity pass — propagate today's paper-tab improvements to the Live Trading tab. Three issues found and fixed: **(a)** Live tab had no headline value tile (`portfolio_value` was paper-only). Live now emits `portfolio_value = Σ(qty × marked_price)` over the live snapshot, labelled **"Holdings Value"** instead of "Portfolio Value" because the web process can't read the Dhan account cash balance (FRD B.2 — only the worker calls Dhan). The label also makes the difference explicit so the user doesn't read the live tile as full PV. **(b)** `web.views.day_grouped_trade_log` was hardcoded to `paper_pnl_daily` even when called with `prefix='live'`, so the live trade-log portfolio value column was reading paper unrealized. Now uses `{prefix}_pnl_daily` consistently. **(c)** First metric on the Performance Summary read "Overall **Paper** Profit" on both tabs; now switches to "Overall **Live** Profit" when `prefix == 'live'`. Test updated: `test_summary_omits_portfolio_value_for_live` flipped to `test_summary_emits_holdings_value_for_live` asserting the new field + label. Suite 165/165. | User request: "make sure all these changes we did today are copied to live as well". |
| 2026-04-26 | CSS specificity bug: `table.data td { color: var(--fg); }` (specificity 0,1,2) was overriding `.pos-cell { color: var(--ok); }` (0,1,0), so the green/red text tint never applied inside any data table — Performance Summary and Current Book values rendered as plain grey even when the cell had a `pos-cell` / `neg-cell` class. The 12% rgba background tint did apply but was too subtle to see at a glance. Fixed by adding higher-specificity selectors `table.data td.pos-cell` / `table.data td.neg-cell` (0,2,2) so the colour wins inside data tables; the original `.pos-cell` / `.neg-cell` rules are kept as a fallback for any non-table use. | User feedback: "conditional formatting in these tables" — Performance Summary still rendered as plain text after the prior commit added the `tone` field. |
| 2026-04-26 | KPI "Unrealized P&L" tile and the Current Paper Trading Book per-row "Unrealized PnL" diverged between refresh ticks — the tile read `paper_pnl_daily.unrealized` (a once-a-minute snapshot written by `paper_mtm_refresh_job`) while the book computed `(marked - avg_cost) * qty` live in `book_rich` from `live_ltp`. As GUJCOTEX moved between the minute the snapshot was written and the next render, the two sources disagreed (e.g. tile +89.43 vs book -268.29 in a single rendered page). Fixed by sourcing `today_unrealized` in `web.views._summary` from `sum(b['unrealized_pnl'] for b in book_rich(conn, prefix))` so the tile and the table share one live computation. `today_mtm` and `cumulative` reuse the live unrealized for the same parity. `paper_pnl_daily.unrealized` is still written by the refresh job and powers the historical chart series, where its once-a-minute cadence is appropriate. New regression: `tests/test_paper_engine.py::test_summary_unrealized_matches_current_book_total` plants a deliberately stale `paper_pnl_daily` row and asserts the tile reads the live value instead. Suite 164/164. | User feedback: "unrealized PnL not matching across the dashboard". |
| 2026-04-26 | Three paper-tab UX tweaks. **(1)** "Today Status" single-row table (a snapshot of the current session that mostly duplicated KPI tiles + the Current Book) was removed; the "Current Paper Trading Book" table now lives in that slot, immediately under the KPI rows. **(2)** Performance Summary metric grid now uses conditional formatting — each row gets a `tone` field in `views.performance_grid` (`signed`, `good_when_positive`, `bad_when_positive`, `ratio_tone`) and the template applies `pos-cell` / `neg-cell` styling so signed P&L values, win/loss ratios, drawdowns, and streaks read at a glance. Neutral counts (Closed Trades, Open Positions, Duration of Max Drawdown) stay uncoloured. **(3)** Daily / Cumulative P&L Plotly charts went blank after the first 30s HTMX poll: the chart-init `<script>` was outside `<section class="tab-body">`, so HTMX's `hx-select=".tab-body"` stripped it from every swap. Moved the script inside the section; added a `typeof Plotly === 'undefined'` guard plus an empty-series guard so a slow CDN or a fresh-install no-data state degrades gracefully instead of throwing. | User feedback iterating through the paper tab. |
